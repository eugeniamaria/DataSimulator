...
void ComputeThetasA(double *TB_kn, double alpha, double gamma, double MN, size_t NS, size_t K, size_t I ){
  ...
  for (i=0; i<I; i++){
    ...
    sumi=0;
    for (k=0; k<K; k++){
      cout << "Population:" << k << " -- " << TB_knt[i*K+k] << endl;
      sumi = sumi + TB_knt[i*K+k];
    }
    cout << "Sum = " << sumi << endl;
    }
...
{  
  ...
  for (i=0; i< rows; i++){
    ...
    for (j = 0; j< cols; j++){if (temp[j] < 1e-16){res[i*cols + j]=0;} else{res[i*cols + j] = temp[j];}}
  }
